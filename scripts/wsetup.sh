...
export   WATTOSCLASSDIR=$WATTOSROOT/build
...
# hostname shows Stella.local or stella.cmbi.umcn.nl
# lower case hostname on cygwin and upper case on windows.
# disabled this more advanced way as gawk is not always present on Mac.
#set wattoshost = (`hostname|gawk -F'[.]' '{print tolower($1)}'| sed -e 's/[-0-9]//g'`)
#set wattoshost = (`hostname`)
