...
import Wattos.Soup.ComparatorTripletByAtom;
...
public class Triplet extends PseudoAtom {
    ...
    /** True for the models in which the assignment with the lowest overall energy is favoured 
    ...
    public boolean deassign(
        ...
        for ( int r=0; r<restraintRidList.size();r++) {
            ...
            if ( ! compileViolationsTripletPerRestraint(rstrtIdx,dc,
...
//        float perViolationOverMultiModelCriterium = (100f*countViolationsAboveThresMultiModel)/modelCount;
        ...
        if ( ( maxViolation >= single_model_violation_deassign_criterium ) ||
             needsDeassignmentByMultiModelCriteria) {
            deassigned = true;        
            for ( int r=0; r<restraintRidList.size();r++) {
                int rstrtIdx = restraintRidList.getQuick(r);
    ...
    /** Transfer the info in perModelEnergy and totalEnergy to
     *energyLowState, modelsFavoured etc. 
     */
    ...
    /** Returns the assignment statistics given the number of models in entry.
     *After this call the list will be sorted by the first atom in each pseudo atom.
     */
    public static boolean toSTAR( ArrayList list, Atom atom, int model_count, 
            SaveFrame sF, AssignStereo aS ) {
...
//        float per_ass;
        ...
        /**
        if ( i != 0 ) {
            General.showCodeBug("Expected first row to be numbered zero but is: " + i);
            return false;
        }
         */
    ...
    public boolean compileEnergyTripletPerRestraint( int rstrtIdx, DistConstr dc, 
            boolean swapState, PseudoLib pseudoLib) {
        ...
        for (int modelId=0;modelId<modelCount;modelId++) {
            ...
            /**
            General.showDebug("triplet: " + orderAssigned + 
                " restraint:   " + rstrtIdx + " state: " + state + " model: " + modelId +
    ...
    /** Calculates the energy contribution of one restraint over all models
                given the assignment state of the triplet to be assigned. */    
    public boolean compileViolationsTripletPerRestraint( int rstrtIdx, 
            DistConstr dc, 
            float single_model_violation_deassign_criterium,
            float multi_model_violation_deassign_criterium,
            float multi_model_rel_violation_deassign_criterium
        ...
        if (( ! Defs.isNull(lower)) &&
            ( ! Defs.isNull(upper)) &&
            ( lower > upper )) {
            General.showWarning("Found lower > upper: " + lower + " > " + upper + " ignoring restraint in Triplet.compileViolationsTripletPerRestraint"); 
            return true;
        }
        ...
        for (int modelId=0;modelId<modelCount;modelId++) {
            ...
            if ( viol > multi_model_violation_deassign_criterium ) {
                if ( modelId == 0 ) {
                    General.showDebug("rstrtIdx: " + rstrtIdx + " modelId: " + modelId + 
                        " upper: " + upper + " value: " + value + " viol: " + viol);
                }
            }             
        }
        // Needs to be compiled per restraint. Might be set to true multiple times.
        float perViolationOverMultiModelCriteriumThisRestraint = 
                (100f*countViolationsAboveThresMultiModelThisRestraint)/modelCount;
