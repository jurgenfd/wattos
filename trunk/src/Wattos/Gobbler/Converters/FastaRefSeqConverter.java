 ...
 *WARNING: 37,777 is the length of the largest sequence in this mouse db.
 *
 ...
 */
public class FastaRefSeqConverter {
...
//    private String regexp_nucelotides_only;
    ...
    public FastaRefSeqConverter() {
        /** Using new regexp in standard java 1.4...
         */
        try {
            // Should match: ">gi|4507653|ref|NP_000358.1| thiopurine S-methyltransferase [Homo sapiens]"
            regexp_summary = "^>gi\\|(\\d+?)\\|(.+?)\\|(.+?)\\|(.*)$";
...
>gi|4507653|ref|NP_000358.1| thiopurine S-methyltransferase [Homo sapiens]
    ...
    public boolean parseSummaryLine( String line ) {
        ...
        //String line         = ">gi|4507653|ref|NP_000358.1| thiopurine S-methyltransferase [Homo sapiens]";
        //                            1       2   3            4
        if ( ! m_summary.group(2).equals("ref" )) {
            General.showError("Second group didn't match 'ref' in: " + line);
            status = false;
        }
        orfid.orf_db_name       = "ref";        
        orfid.orf_db_id         = m_summary.group(3).trim();
        orfid.molecule_name     = m_summary.group(4).trim();
        // Get the pesty bars out.
        orfid.molecule_name     = orfid.molecule_name.replaceAll("\\|", ",");
    ...
    public static void main(String[] args) {
        ...
        General.showOutput( "Starting FastaRefSeqConverter.convert version 1");
        FastaRefSeqConverter c = new FastaRefSeqConverter();
        
        if ( testing ) {
            //String line         = ">gi|4507653|ref|NP_000358.1| thiopurine S-methyltransferase [Homo sapiens]";
            String line         = ">gi|51474678|ref|XP_375603.2| PREDICTED: similar to comment for location 3447"+
                                  "-3655: BLASTX gi|103290|pir||S16356 ovo protein - fruit fly (Drosophila melanogaster), PVal= 3.8e-47 [Homo sapiens]";
            c.parseSummaryLine(line);
            General.showOutput("line: [" + line + "]");
            General.showOutput("orf: [\n" + c.orf.toFasta() + "]");            
        } else {
            if ( args.length != 2) {
                General.showOutput("\nUsage:  java Wattos.Gobbler.Converters.FastaRefSeqConverter " +
