...
//            General.showOutput("In getPDBIdFromMRFileByDetail found size results: " + ids.size());
    ...
    public int getNextSequenceId( String table_name ) {
        ...
        /* Old method useful for DB other than Oracle perhaps
         *
        int count = stmt.executeUpdate(
            "UPDATE sequence_id SET seq_id = seq_id + 1");
        if ( count != 1 ) {
            String message = 
                "ERROR: in SQL_Episode_II.getNextSequenceId found update of seq_id failed, it returned ["+count+
                "] insertions which should have been exactly one.\n"+
                "ERROR: Most likely the database wasn't properly initialized.\n";
            throw new SQLException( message );
        } 
        rs = stmt.executeQuery(
            "SELECT MAX(seq_id) AS seq_id FROM sequence_id");
        if (rs.next()) 
        {
            nextId = rs.getInt("seq_id");
            // Has to follow the column on which to operate.
            if (rs.wasNull())
                throw new SQLException( "ERROR: in SQL_Episode_II.getNextSequenceId found next id obtained was null for this entry" );
        } else 
           throw new SQLException( "ERROR: in SQL_Episode_II.getNextSequenceId found no result after update and query of sequence_id" );
            rs.close(); 
            stmt.close();
         */
    }
    ...
    /** Delete all images where entry id is as given. 
     * This method uses auto committing.
     * @param entry_id The entry number for which all images are to be deleted.
     * @return Return value indicates success.
     *
    public boolean deletePicturesOfEntry(int entry_id) {
        try 
        {
            Statement stmt = conn.createStatement();  
            ...
            int count = stmt.executeUpdate(
                "DELETE FROM " + image_table + General.eol +
                "WHERE entry_id="+entry_id);
            if ( count < 1 ) {
                //General.showWarning("no images where deleted for entry_id :"+entry_id);
            } else 
            {
                General.showOutputNoEol("The number of images deleted for entry_id : ");
                Parameters p = new Parameters(); // Printf parameters
                Format.printf("[%9d] is: [%9d]\n",
                         p.add( entry_id).add( count ));
            }
            // Do these need to be executed before a return, or is it just 
            // nice to do so?
            // Any warnings generated by the sql connection?
        ...
        {
            General.showError("in SQL_Episode_II.deletePicturesOfEntry found Database access failed.\n" + e);
            return false;
        }
        return true;
    }
    ...
    public DBEntry.Retval getEntryByPDBId( String pdb_id ) {
        ...
        try {
            ...
            if (rs.next()) 
            {
                ...
                String query = 
                    "INSERT INTO " + entry_table + " (entry_id, bmrb_id, pdb_id) \n"+
                    "VALUES ( "+ ret.entry_id + ", NULL, '" + pdb_id + "')";                    
                //General.showOutput("query is:\n" + query);
                
                // Not needed.
                //stmt = conn.createStatement();
                int count = stmt.executeUpdate( query );              
                if ( count != 1 ) {
                    General.showError("in SQL_Episode_II.getEntryByPDBId found insertion returned ["+count+
                        "] insertions which should have been exactly one.");
                    rs.close(); 
                    stmt.close();		// All done with that result set
                    rollbackTransaction();
