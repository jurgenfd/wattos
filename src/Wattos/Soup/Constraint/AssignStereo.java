...
/**
 * Swaps or deassigns stereospecific protons. For an explanation see {@link #doAssignStereo}. 
 * Routines are tested by the JUnit test for Wattos.Star.NMRStar.File31.
 * @author jurgen
 * @see #doAssignStereo
 */
public class AssignStereo {
    ...
    /**
     * Swap stereospecific assignments if the swapped state has a lower energy 
     * (sum of the squared bound violations) in more than a given percentage of models. 
     * An atom(-group) pair and the representing pseudo is called a triplet in this
     * routine.
     * <P>
     ...
     * single_model_violation_deassign_criterium Angstrom,
     * then all restraints involved with this triplet will be deassigned for the
     * ambiguity in this triplet. If a single restraint violates more than parameter 
     * <code>multi_model_rel_violation_deassign_criterium</code>
     * percent of the models more than <CODE>multi_model_violation_deassign_criterium</CODE> 
     * Angstrom then all restraints involved with this triplet will be deassigned for the
     * ambiguity in this triplet.
     * 
     * <P> 
     ...
     *  
     * <UL>
     * <LI>The algorithm is akin to the C code written in Aqua (AquaAssign module) 
     * and the Fortran code that Alexandre Bonvin (Utrecht University) wrote for 
     * the statement "aria flip ...".
     * <LI>Atoms/atomgroups A that are considered to be swappable are:
     * CH2_OR_NH2, TWO_METHYL, AROMAT_2H, but not METHYL, TWO_NH2_OR_CH2, and AROMAT_4H.
     * See {@link Wattos.Soup.PseudoLib} for the exact definitons.
     * </UL>
     * <BR>
     * <P>
     * This implementation in Wattos is similar to but not the same as the one defined in:<BR>
     * Nederveen,A.J., Doreleijers,J.F., Vranken,W., Miller,Z., Spronk,C.A., Nabuurs,S.B., 
     * Guntert,P., Livny,M., Markley,J.L., Nilges,M., Ulrich,E.L., Kaptein,R. and
     * Bonvin,A.M. (2005). RECOORD: a recalculated coordinate database of 500+
     * proteins from the PDB using restraints from the BioMagResBank.
     * <I>Proteins</I> <B>59</B>, 662-672.
     * See {@link <a href="http://www.ncbi.nlm.nih.gov/entrez/query.fcgi?cmd=Retrieve&db=PubMed&list_uids=15822098&dopt=Abstract">here</a>}.
     * <BR>
     * @param energy_abs_criterium Absolute energy difference between the two assignment states that will cause a 
     * swap to be made. The energy is averaged by the number of models. Recommended value is 0.1 Ang**2.
     * Set to a negative number to prevent swaps.
     * @param energy_rel_criterium Relative energy difference (as a percentage) between the two assignment states that will cause a 
     * swap to be made. Recommended value is 0 % and let the absolute energy difference criterium 
     * <CODE>energy_abs_criterium</CODE> do its job.
     * @param model_criterium Percentage of models that need to have the state swapped by the energy difference 
     * between the two assignment states that will cause a swap to be made. 
     * Recommended value is 49 % (or higher).
     * @param single_model_violation_deassign_criterium Deassignment criterium for single model violation (1.0 Angstrom suggested). Set this value
     * and the parameter <CODE>multi_model_violation_deassign_criterium</CODE> to a high 
     * positive number (e.g. 999.9) to prevent any deassignment.
     * @param multi_model_violation_deassign_criterium Deassignment criterium for multiple model violation (0.5 Angstrom suggested)
     * @param multi_model_rel_violation_deassign_criterium Deassignment criterium for multiple model violation percentage (50 % suggested)
     * @param outputFileName File name for the STAR formatted result file (stereo_assign.str suggested)
     * @return <CODE>true</CODE> for success
     * @see Wattos.Soup.PseudoLib
     */    
    public boolean doAssignStereo(
            float energy_abs_criterium, 
            float energy_rel_criterium, 
            float model_criterium,
            float single_model_violation_deassign_criterium,
            float multi_model_violation_deassign_criterium,
        ...
        ArrayList tripletList = new ArrayList();
        ...
        for ( int t=0;t<tripletList.size();t++) {
            ...
            if ( ! triplet.assign(          energy_abs_criterium, 
        ...
        for ( int t=0;t<tripletList.size();t++) {
            ...
            if ( ! triplet.deassign(        single_model_violation_deassign_criterium, 
...
//        TagTable tTAssign = (TagTable) sF.get(1);
        ...
        tTAssignIntro.setValue(i, tagNameCriterium_deassign_single_model_violation,             single_model_violation_deassign_criterium);
        tTAssignIntro.setValue(i, tagNameCriterium_deassign_multiple_model_violation,           multi_model_violation_deassign_criterium);
        tTAssignIntro.setValue(i, tagNameCriterium_deassign_multiple_model_percentage,          multi_model_rel_violation_deassign_criterium);        
    ...
    /**
     * Simple count of the restraints associated with this triplet that are
     * uniquely assigned with the exception of the ambiguity possible from the
     * given triplet.
    ...
    /**
     * Remove all restraints from triplet that are ambi in the triplet because
     * those types of restraints don't need to be considered.
     * E.g. for triplet VAL QG a restraint VAL QG <-> GLY H will be removed
     * but a restraint VAL MG <-> GLY H will not.
     * @param triplet 
     * @return <CODE>true</CODE> for success
     */
    public boolean removeRestraintsAmbiInTriplet(Triplet triplet ) {
        for ( int r=0;r<triplet.restraintRidList.size();r++) {
            ...
            BitSet atomDCAtomRidSet = SQLSelect.selectBitSet(ui.dbms, 
                dc.distConstrAtom,
    ...
    /**
     * Returns true when the restraint is uniquely assigned with the exception 
     * of the ambiguity possible from the given triplet.<BR>
     * The algorithm:<BR>
     * <PRE>
     *  Get list of atoms L involved in restraint.
     ...
     *  return true
     * </PRE>
     * Notes: An atom A is unique if it is not in any pseudoatom considered or if
     * it is in such pseudoatom and all other members of the pseudoatom are also
     * in L.
     * @param dcRid Distance constraint id
    ...
    public int isUniqueAssignedRestraint( int dcRid, Triplet triplet) {
        ...
        General.showDebug("     for triplet: " + triplet.toString());
        General.showDebug("Atom rids involved: " + PrimitiveArray.toString( PrimitiveArray.toIntArrayList(atomRidSet)));
         */
        BitSet atomRidSetTriplet = PrimitiveArray.toBitSet( triplet.atomRids );        
        // Exclude the atoms in this triplet
    ...
    public boolean initConvenienceVariablesStar() {
        ...
        try {
            ...
            tagNameSwap_count                               = (String) ((ArrayList)starDict.toStar2D.get( "stereo_assignments","_Stereo_assignments.Swap_count"                        )).get(StarDictionary.POSITION_STAR_TAG_NAME);
            tagNameSwap_percentage                          = (String) ((ArrayList)starDict.toStar2D.get( "stereo_assignments","_Stereo_assignments.Swap_percentage"                        )).get(StarDictionary.POSITION_STAR_TAG_NAME);
            tagNameDeassign_count                           = (String) ((ArrayList)starDict.toStar2D.get( "stereo_assignments","_Stereo_assignments.Deassign_count"                        )).get(StarDictionary.POSITION_STAR_TAG_NAME);
            tagNameDeassign_percentage                      = (String) ((ArrayList)starDict.toStar2D.get( "stereo_assignments","_Stereo_assignments.Deassign_percentage"                        )).get(StarDictionary.POSITION_STAR_TAG_NAME);
            tagNameModel_count                              = (String) ((ArrayList)starDict.toStar2D.get( "stereo_assignments","_Stereo_assignments.Model_count"                        )).get(StarDictionary.POSITION_STAR_TAG_NAME);
            tagNameTotal_energy_low_states                  = (String) ((ArrayList)starDict.toStar2D.get( "stereo_assignments","_Stereo_assignments.Total_energy_low_states"                        )).get(StarDictionary.POSITION_STAR_TAG_NAME);
            tagNameTotal_energy_high_states                 = (String) ((ArrayList)starDict.toStar2D.get( "stereo_assignments","_Stereo_assignments.Total_energy_high_states"                        )).get(StarDictionary.POSITION_STAR_TAG_NAME);
            tagNameCriterium_absolute_energy_difference            = (String) ((ArrayList)starDict.toStar2D.get( "stereo_assignments","_Stereo_assignments.Criterium_absolute_energy_difference"                        )).get(StarDictionary.POSITION_STAR_TAG_NAME);
            tagNameCriterium_relative_energy_difference            = (String) ((ArrayList)starDict.toStar2D.get( "stereo_assignments","_Stereo_assignments.Criterium_relative_energy_difference"                        )).get(StarDictionary.POSITION_STAR_TAG_NAME);
            tagNameCriterium_percentage_models_favoring            = (String) ((ArrayList)starDict.toStar2D.get( "stereo_assignments","_Stereo_assignments.Criterium_percentage_models_favoring"                        )).get(StarDictionary.POSITION_STAR_TAG_NAME);
            tagNameCriterium_deassign_single_model_violation       = (String) ((ArrayList)starDict.toStar2D.get( "stereo_assignments","_Stereo_assignments.Criterium_deassign_single_model_violation"                        )).get(StarDictionary.POSITION_STAR_TAG_NAME);
            tagNameCriterium_deassign_multiple_model_violation     = (String) ((ArrayList)starDict.toStar2D.get( "stereo_assignments","_Stereo_assignments.Criterium_deassign_multiple_model_violation"                        )).get(StarDictionary.POSITION_STAR_TAG_NAME);
            ...
            tagNameLabel_entity_ID                          = (String) ((ArrayList)starDict.toStar2D.get( "stereo_assignments","_Stereo_assignments.Label_entity_ID"                        )).get(StarDictionary.POSITION_STAR_TAG_NAME);
            tagNameAssignment_ID                            = (String) ((ArrayList)starDict.toStar2D.get( "stereo_assignments","_Stereo_assignments.Assignment_ID"                        )).get(StarDictionary.POSITION_STAR_TAG_NAME);
            tagNameSwapped                                  = (String) ((ArrayList)starDict.toStar2D.get( "stereo_assignments","_Stereo_assignments.Swapped"                        )).get(StarDictionary.POSITION_STAR_TAG_NAME);
            tagNamePercentage_models_favoring               = (String) ((ArrayList)starDict.toStar2D.get( "stereo_assignments","_Stereo_assignments.Percentage_models_favoring"                        )).get(StarDictionary.POSITION_STAR_TAG_NAME);
            tagNamePercentage_energy_difference             = (String) ((ArrayList)starDict.toStar2D.get( "stereo_assignments","_Stereo_assignments.Percentage_energy_difference"                        )).get(StarDictionary.POSITION_STAR_TAG_NAME);
            tagNameEnergy_difference                        = (String) ((ArrayList)starDict.toStar2D.get( "stereo_assignments","_Stereo_assignments.Energy_difference"                        )).get(StarDictionary.POSITION_STAR_TAG_NAME);
            tagNameEnergy_high_state                        = (String) ((ArrayList)starDict.toStar2D.get( "stereo_assignments","_Stereo_assignments.Energy_high_state"                        )).get(StarDictionary.POSITION_STAR_TAG_NAME);
            tagNameEnergy_low_state                         = (String) ((ArrayList)starDict.toStar2D.get( "stereo_assignments","_Stereo_assignments.Energy_low_state"                        )).get(StarDictionary.POSITION_STAR_TAG_NAME);
            tagNameRestraint_count                          = (String) ((ArrayList)starDict.toStar2D.get( "stereo_assignments","_Stereo_assignments.Restraint_count"                        )).get(StarDictionary.POSITION_STAR_TAG_NAME);
            tagNameRestraint_ambi_count                     = (String) ((ArrayList)starDict.toStar2D.get( "stereo_assignments","_Stereo_assignments.Restraint_ambi_count"                        )).get(StarDictionary.POSITION_STAR_TAG_NAME);
            tagNameDeassigned                               = (String) ((ArrayList)starDict.toStar2D.get( "stereo_assignments","_Stereo_assignments.Deassigned"                        )).get(StarDictionary.POSITION_STAR_TAG_NAME);
            tagNameMaximum_violation                        = (String) ((ArrayList)starDict.toStar2D.get( "stereo_assignments","_Stereo_assignments.Maximum_violation"                        )).get(StarDictionary.POSITION_STAR_TAG_NAME);
            tagNameViolation_single_model_criterium_count   = (String) ((ArrayList)starDict.toStar2D.get( "stereo_assignments","_Stereo_assignments.Violation_single_model_criterium_count"                        )).get(StarDictionary.POSITION_STAR_TAG_NAME);
        ...
        if ( false ) {
            ...
            tagNameLabel_entity_ID,
            tagNameAssignment_ID,
            tagNameSwapped,
            tagNamePercentage_models_favoring,
            tagNamePercentage_energy_difference,
            tagNameEnergy_difference,
            tagNameEnergy_high_state,
            tagNameEnergy_low_state,
            tagNameRestraint_count,
            tagNameRestraint_ambi_count,                  
            tagNameDeassigned,
            tagNameMaximum_violation,
            tagNameViolation_single_model_criterium_count,
...
//        int                 RID;
        try {            
            ...
            namesAndTypes.put( tagNameCriterium_deassign_multiple_model_violation,               new Integer(Relation.DATA_TYPE_FLOAT));
            namesAndTypes.put( tagNameCriterium_deassign_multiple_model_percentage,              new Integer(Relation.DATA_TYPE_FLOAT));
