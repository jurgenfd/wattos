...
//    public String tagNameTA_constraint_stats_TA_constraint_stats_list_ID;
    ...
    static {
        ...
        explanation =
                ...
                "*  "+i+++" * Sum of violations in degrees.                                                             \n" +
        ...
        explanation = explanation +
                ...
                "*  "+i+++" * Torsion angle name where available.                             \n" +
                "*  "+i+++" *     First atom's:                                               \n" +
                "*  "+i+++" * Chain identifier (can be absent if none defined)                \n" +
                "*  "+i+++" * Residue number                                                  \n" +
                "*  "+i+++" * Residue name                                                    \n" +
                "*  "+i+++" * Name of (pseudo-)atom                                           \n" +
                "*  "+i+++" *  Second thru fourth atom's identifiers occupy columns 7 thru 18.\n";
        
        i = 19;
        explanation = explanation +
            "*  "+i+++" * Lower bound (degrees)                                               \n" +
            "*  "+i+++" * Upper bound (degrees)                                               \n" +
            "*  "+i+++" * Average angle in ensemble of models                                 \n" +
            "*  "+i+++" * Minimum angle in ensemble of models (counter clockwise from range)  \n" +
            "*  "+i+++" * Maximum angle in ensemble of models         (clockwise from range)  \n" +
...
//      General.showDebug("Found ATTRIBUTE_SET_SUB_CLASS:"+Strings.toString(ATTRIBUTE_SET_SUB_CLASS));
    ...
    public boolean toXplor(BitSet todo, String fn, String atomNomenclature) {
        ...
        for (int listRID = todo.nextSetBit(0); listRID >= 0; listRID=todo.nextSetBit(listRID+1)) {
            fileCount++;
            BitSet ridsCDIH = SQLSelect.selectBitSet(dbms, 
                    constr.cdih.mainRelation, 
...
//        General.showDebug("Models: " + PrimitiveArray.toString( selectedModelArray ));
        ...
        if ( unlinkedAtomCount > 0 ) {
            General.showWarning("Will skip violation calculation for number of constraints: " + unlinkedAtomCount + " because not all atoms are linked for them." );
        }
        // FOR EACH CONSTRAINT
        for (int currentCDIHId = todoCDIHFiltered.nextSetBit(0);currentCDIHId>=0;currentCDIHId = todoCDIHFiltered.nextSetBit(currentCDIHId+1)) {
            ...
            for ( int currentModelId=0; currentModelId<selectedModelArray.length; currentModelId++) {
                ...
                if ( Defs.isNull( value ) ) {
                    General.showError("Failed to calculate the value for constraint: " + currentCDIHId + " in model: " + (currentModelId+1) + " will NOT try other models.");
                    return false;
                }
                float viol = (float) Geometry.violationAngles( (double) low, (double) upp, (double) value);
                boolean[] isViol = Geometry.isLowUppViolationAngles( 
                        (double) low, (double) upp, (double) value, (double) viol);
...
//      General.showDebug("Calculated values for number of constraints: " + cdihCount + " in number of models: " + selectedModelArray.length);
     ...
     * Make sure that the violations have been gathered before.
     * DistConstr.calcViolation
     */
    public SaveFrame toSTAR( BitSet todoCDIH, int listNumber, int currentCDIHListId,
            boolean showViolations, float cutoffValue ) {
        cdih=constr.cdih;
        ...
        IndexSortedInt indexListMain = (IndexSortedInt) cdih.mainRelation.getIndex(Constr.DEFAULT_ATTRIBUTE_SET_CDIH_LIST[ RelationSet.RELATION_ID_COLUMN_NAME ], Index.INDEX_TYPE_SORTED);
...
//                tagNameTA_constraint_stats_list_Constraint_file_ID            == null ||
                ...
                tagNameTA_constraint_stats_list_Viol_max                      == null ||
                tagNameTA_constraint_stats_list_Viol_rms                      == null ||
                ...
                tagNameTA_constraint_stats_list_Details                       == null ||
                tagNameTA_constraint_stats_Restraint_ID                       == null ||
                tagNameTA_constraint_stats_Torsion_angle_name                 == null ||
                tagNameTA_constraint_stats_Entity_assembly_ID_1               == null ||
                tagNameTA_constraint_stats_Comp_index_ID_1                    == null ||
                tagNameTA_constraint_stats_Comp_ID_1                          == null ||
                tagNameTA_constraint_stats_Atom_ID_1                          == null ||
                tagNameTA_constraint_stats_Entity_assembly_ID_2               == null ||
                tagNameTA_constraint_stats_Comp_index_ID_2                    == null ||
                tagNameTA_constraint_stats_Comp_ID_2                          == null ||
                tagNameTA_constraint_stats_Atom_ID_2                          == null ||
                tagNameTA_constraint_stats_Entity_assembly_ID_3               == null ||
                tagNameTA_constraint_stats_Comp_index_ID_3                    == null ||
                tagNameTA_constraint_stats_Comp_ID_3                          == null ||
                tagNameTA_constraint_stats_Atom_ID_3                          == null ||
                tagNameTA_constraint_stats_Entity_assembly_ID_4               == null ||
                tagNameTA_constraint_stats_Comp_index_ID_4                    == null ||
                tagNameTA_constraint_stats_Comp_ID_4                          == null ||
                ...
                tagNameTA_constraint_stats_Angle_average                      == null ||
                tagNameTA_constraint_stats_Angle_minimum                      == null ||
                tagNameTA_constraint_stats_Angle_maximum                      == null ||
