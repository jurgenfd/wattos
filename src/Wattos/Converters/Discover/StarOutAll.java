...
/** StarOutAll class is used to produce Star file output from Discover input
 * files.
 */  
...
public class StarOutAll{
    ...
    /** Main data resides in this vector. Vectors passed from DiscoverParserAll by calling corresponding pop method
     */    
    static Vector stack;
     ...
     */    
    static DiscoverParserAll parser = new DiscoverParserAll(System.in);    
    ...
    public static void init() {
        ...
        //Vectors passed from DiscoverParserAll by calling corresponding pop method
        stack = null;
        cmtStack = null;
        errStack = null;
        ...
        parser.init();
        DiscoverParserAll.ReInit( System.in );
    }
    ...
    static public StarFileNode convertToStarFileNode( String input,  int type, int star_version ) {
        ...
        //pop result from DiscoverParserAll
        stack    = parser.popResult();
        cmtStack = parser.popComment();
        errStack = parser.popError();
        //General.showOutput("Finished parsing");

        /** Gets results from DiscoverParserAll generate the STAR structure in memory and
        ...
        BlockNode discover_block       = new BlockNode("data_discover");
        ...
        discover_block.addElement(constraints);
        sfn.addElement(discover_block);
    ...
    public static void showUsage() {
        General.showError("Usage:java Wattos.Converters.Discover.StarOut <input_file> <output_file> <data type id>");
...
//            looprownode_constraint.addElement(new DataValueNode( NmrStar.getPossibleValue( treeNode.entry.getValue( "potential_function_exponent" ))));
    ...
    /** Residual dipolar coupling section. WARNING: Not implemented yet upstream.
     * @param save_frame_node_constraint A reference to the saveframe node of the constraints to which the data should
     * be added.
     */    
...
//                        looprownode_atom.addElement(    new DataValueNode( STAR_EMPTY ));
                        ...
                        switch ( star_version ) {
                            ...
                            default: {
                                General.showError("code bug Converters.Discover.saveDataDistance Unknown nmr-star format: " + star_version );
                            }                            
                        }                                                
...
//                        looprownode_distance.addElement(    new DataValueNode( ppm2 ));
                        looprownode_distance.addElement(    new DataValueNode( NmrStar.getPossibleValue( treeNode.entry.getValue( "dist" ))));
...
//                        looprownode_distance.addElement(    new DataValueNode( ppm2 ));
     ...
     * line.
     * @param args First argument : input file (discover)
     * Second argument: output file (STAR)
     * Third argument : data type (e.g. 1 for distances)
     *
     * <PRE>Usage:java Wattos.Converters.Discover.StarOutAll <input_file> <output_file> <[1-3]></PRE>
     */    
    public static void main(String[] args) {
        ...
        //check for arguments, require one Discover template file (.config file) followed by
        //the Discover file block to be processed
        if (args.length != 3) {
            showUsage();
        }
