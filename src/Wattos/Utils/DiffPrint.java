...
import Wattos.Utils.Diff;

/** Part of a simple framework for printing change lists produced by <code>Diff</code>. 
 * @see DiffPrint
 * @see Diff
...
public class DiffPrint {
  ...
  /** Read a text into an array of String. 
   */
  public static String[] slurpString(String input) throws IOException {
  ...
  /** Most convenient method for printing diffs between 2 strings
   *line by line.
   */
    public static boolean printDiff( String a, String b) {
        try {
            String[] a2 = DiffPrint.slurpString(a);
            String[] b2 = DiffPrint.slurpString(b);
            Diff d = new Diff(a2,b2);
            Diff.change script = d.diff_2(false);
            if (script == null) {
                General.showOutput( "No differences");
            } else {
                NormalPrint p = new NormalPrint(a2,b2);
                p.print_script(script);
            }
        } catch ( Exception e ) {
            General.showThrowable(e);
            General.showError("slurpString text.");
            return false;
        }
        return true;
    }
  ...
  public static void main(String[] argv) throws IOException {
    ...
    {
        General.showOutput("USAGE: java Wattos.Utils.DiffPrint [-e] filename_1 filename_2");
        General.showOutput("Meaning of option:");
        General.showOutput("-e Show output as ed script in stead of regular output.");
        System.exit(1);
    }
