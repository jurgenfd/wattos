...
//        General.showOutput("PID  = " + pid);
    ...
    public static String getRevision(String rootName) {
        ...
        if (rootDirStr == null) {
            General.showError("Without set " + rootName + " variable can't getRevision");
            return Defs.NULL_STRING_NULL;
        }
        String cmd = "cd " + rootDirStr + "; git rev-parse --short HEAD";

        String[] result = OSExec.getstatusoutput(cmd);
        if (!result[0].equals("0")) {
            General.showError("Failed to run cmd: [" + cmd + "]");
            return Defs.NULL_STRING_NULL;
        }
        ArrayList<String> lineList = Strings.getLines(result[1]);
        if (lineList.size() != 1) {
            General.showError("Failed to get result from: [" + result[1] + "]");
            return Defs.NULL_STRING_NULL;
        }
...
//      at   = "(3676) 29-Oct-08 15:36:22
        ...
        String header =
                "======================================================================================================\n" +
                "| Wattos: Programs for Structural Biology & NMR spectroscopy " + versionStr + "JFD,GWV,ELU " + copyright_years + " |\n" +
                "======================================================================================================";
